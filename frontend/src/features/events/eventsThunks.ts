...
export const fetchOneEvent = createAsyncThunk<Event, string>('events/fetchOne', async (id) => {
...
export const removeEvent = createAsyncThunk<void, string>('events/remove', async (id) => {
  await axiosApi.delete(`/events/${id}`);
});

export const updateEvent = createAsyncThunk<
  void,
  { id: string; event: EventMutation },
  { rejectValue: ValidationError }
>('events/update', async ({ id, event }, { rejectWithValue }) => {
  try {
